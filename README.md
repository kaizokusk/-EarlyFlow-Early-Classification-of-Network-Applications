...
│   │
│   ├── phase2_timeseries.py          ← Build X_train/val/test.npy from CSVs
...
│   │
│   ├── data/                         ← Generated by phase2 (not committed)
│   │   ├── X_train.npy               (n_train, 20, 7)  float32
│   │   ├── X_val.npy                 (n_val,   20, 7)  float32
│   │   ├── X_test.npy                (n_test,  20, 7)  float32
│   │   ├── y_train.npy               (n_train,)  int32
│   │   ├── y_val.npy                 (n_val,)    int32
│   │   ├── y_test.npy                (n_test,)   int32
│   │   ├── meta.json                 T_MAX, features, label_map, scaler params
│   │   ├── trig_probas_deep.npy      (n_trig, 20, 4) — generated by build_probas.py
│   │   ├── val_probas_deep.npy       (n_val,  20, 4) — generated by build_probas.py
│   │   └── y_trig.npy                (n_trig,) — trigger set labels
│   │
│   ├── models/                       ← Saved models (not committed)
│   │   ├── rocket.pkl                MiniRocket transformer (1000 kernels)
│   │   ├── classifiers.pkl           20 calibrated Ridge classifiers
│   │   ├── trigger.pkl               CALIMERA trigger at α=0.5
│   │   ├── cost_matrices.pkl
│   │   ├── training_report.json      MiniRocket per-timestamp val accuracy
│   │   ├── deep_{model}_best.pt      Best deep model checkpoint
│   │   ├── cal_{model}.pt            Calibrated deep model weights
│   │   └── cal_{model}.platt.pkl     Platt scaler for deep model
│   │
│   └── eval/                         ← Evaluation outputs
│       ├── sweep_results.json         Per-alpha metrics table
│       ├── pareto_plot.png            Accuracy vs earliness scatter
│       ├── per_timestamp_acc.png      Accuracy growth by packet count
│       ├── f1_per_timestamp.json      Per-class F1/P/R at every timestamp
│       ├── plot_macro_prf1.png        Macro P/R/F1 vs packets plot
│       ├── plot_perclass_f1.png       Per-class F1 vs packets
│       ├── plot_perclass_precision.png
│       ├── plot_perclass_recall.png
│       ├── report_f1_full.txt         sklearn classification_report at every t=1..20
│       ├── report_f1_triggered.txt    Report on CALIMERA-triggered predictions
│       ├── report.md                  Full written summary report
│       ├── compare_table.csv          All models comparison
│       └── compare_plot.png           Grouped bar chart
│
├── early_ml/
│   └── split_file/                   ← Raw input CSVs (not committed — large)
│       ├── cloud_filtered.csv
│       ├── social_media_filtered.csv
│       ├── streaming_filtered.csv
│       └── web_filtered.csv
│
...
```
Raw CSVs (early_ml/split_file/)
         │
         ▼
phase2_timeseries.py
...
1. **Loads** all `*_filtered.csv` files from `early_ml/split_file/`
...
```bash
cd /home/sai/Downloads/19096823
...
# Step 1: Build time-series arrays from raw CSVs
python calimera/phase2_timeseries.py
